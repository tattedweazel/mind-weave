...
- **`app/api/v1/`** — FastAPI route definitions organized by resource:
  ...
  - `workflow_definitions.py` — WorkflowDefinition CRUD, run, run_stream, runs, run logs
...
1. **Validate** — Detect cycles (Kahn's algorithm).
...
**SPA parity — persisted workflow + `run_stream` (HTTP)** — `cd backend && uv run python scripts/run_persisted_workflow_stream_http_e2e.py --workflow-id <uuid>` **GET**s the definition, **POST**s **`run_stream`** like **`ApiClient.runWorkflowStream`**. Optional **`--assert-upsert-body-non-empty`**. Pair with **`uv run python scripts/analyze_workflow_definition.py --workflow-id <same-uuid> --summarize`** (offline DB view). Same doc section as [`analyze_workflow_definition.py`](scripts/analyze_workflow_definition.py).
...
**HTTP `run_stream` and DB sessions** — [`run_workflow_stream`](app/api/v1/workflow_definitions.py) opens the SQLAlchemy session **inside the streaming response body** (via the same factory **`Depends(get_session)`** uses, including test overrides). Request-scoped **`Depends(get_session)`** on that route alone is not enough: the session can be torn down when the handler returns **`StreamingResponse`**, before bytes are sent — the editor then sits on “Running” with no NDJSON (e.g. voice input). Response headers include **`X-Accel-Buffering: no`** so reverse proxies do not buffer chunks.

**Voice input (transcribe) in-process, no browser** — `cd backend && uv run python scripts/run_voice_input_workflow_e2e.py` inserts a **Start → Voice input → Stop** **WorkflowDefinition**, runs **`run_stream`**, and calls **`complete_transcribe_wait`** (same as **`POST .../transcribe-audio`**). Default: **mocks** STT (no stt-bridge) and uses a tiny placeholder WAV. Use **`--real-stt --audio tts`** for a full **TTS → upload → STT** path (TTS and STT bridges must be up; TTS model discovery matches **`tests/test_tts_workflow_e2e_optional.py`**). See the script’s module docstring and [../docs/WORKFLOW_SKILLS.md](../docs/WORKFLOW_SKILLS.md#voice-input-transcribe--skill-transcribe_audio).

**Transcribe File (provider-abstracted) in-process, no browser** — `cd backend && uv run python scripts/run_transcribe_file_workflow_e2e.py` exercises both providers (`local_whisper` + `assemblyai`) and both source modes (saved artifact + runtime upload via `complete_transcribe_wait`). Defaults mock STT and AssemblyAI so no external calls fire. Pass `--real-stt --audio-file …` or `--real-assemblyai --assemblyai-key … --audio-file …` to drive the live bridges. The HTTP twin `scripts/run_transcribe_file_workflow_http_e2e.py` boots a local FastAPI process, exercises both providers through `run_stream`, and verifies the **reattach-stream** endpoint replays persisted state. See the scripts' docstrings and [../docs/WORKFLOW_SKILLS.md](../docs/WORKFLOW_SKILLS.md#transcribe-file-provider-abstracted--skill-transcribe_file).
...
| PUT | `/api/v1/auth/me` | Update current user (requires auth). Body: `{ "settings": {...}, "api_keys": {...} }`. Allow-listed `settings` keys include `avatar_url`, `system_colors`, **`system_palette_id`**, **`theme_mode`** (`light` / `dark` / `system`), and **`preferred_editor_palette_id`** (UUID or `null` for new-workflow default workflow palette). |
...
| POST | `/api/v1/workflow-definitions/{id}/run` | Execute workflow (returns result). Optional body: `{ "input_overrides": { "key": value } }` for required inputs that are null; `{ "output_overrides": { "<node_id>": <json> } }` to force per-node outputs (skips execution for those nodes). |
| POST | `/api/v1/workflow-definitions/{id}/run_stream` | Execute workflow (NDJSON stream). Same optional body fields as `/run`. |
...
| `STT_AUDIO_WAIT_TIMEOUT` | `300.0` | Max seconds a **`transcribe_audio`** step blocks in **`run_stream`** waiting for **`POST .../transcribe-audio`** from the editor. |
...
| `WORKFLOW_RUN_RATE_LIMIT` | `60/minute` | In-process per-IP limit on `POST .../workflow-definitions/{id}/run` and `.../run_stream` combined. Same `N/minute` format. |
