...
| POST | `/api/v1/workflow-definitions/{id}/runs` | Enqueue persisted Build run (**`queued`** immediately). Same optional **`WorkflowRunRequest`** body as **`/run`**. Consume lifecycle on **`GET /api/v1/workflow-runs/{run_id}/events`**. |
