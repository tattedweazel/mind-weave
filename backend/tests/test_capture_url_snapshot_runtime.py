...
def test_png_dimensions_minimal():
    ...
    b64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+lmfkAAAAASUVORK5CYII="
...
@pytest.mark.asyncio
async def test_perform_url_snapshot_playwright_not_installed(monkeypatch):
    ...
    assert "error" in r
    assert r["error"]["type"] == "PLAYWRIGHT_MISSING"
    assert "url-snapshot" in r["error"]["message"]
