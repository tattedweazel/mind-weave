...
- **`src/api/`** — API client and types:
  ...
  - `types.ts` — TypeScript interfaces mirroring backend Pydantic models (source of truth: [`backend/app/domain/schemas/`](../backend/app/domain/schemas/))
...
- From `frontend/`: `npm run test` (Vitest + Testing Library). Before a public release, also run `npm run test:cov` for a V8 coverage report (no global percentage gate; see [docs/Audits/TEST_AUDIT.md](../docs/Audits/TEST_AUDIT.md)).
...
2. **WorkflowEditor** (`src/components/workflow-editor/`): Add color key to [`src/domain/paletteDefaults.ts`](src/domain/paletteDefaults.ts) (`WORKFLOW_PALETTE_COLORS` and `PALETTE_KEY_TO_FAMILY`); create `XxxNodeComp` using `StyledNodeBase`; register in `nodeTypes`; add to **Utilities** palette array; handle in `onDrop`, `appNodeToFlow`, `flowNodeToApp`; enrich in `nodesForFlow`; add to `getSourceOutputType`; update `appEdgeToFlow` target/source handle defaults; add inspector block
3. **PaletteManager** (`src/components/PaletteManager.tsx`): Add to `PALETTE_ENTITIES` (defaults come from `WORKFLOW_PALETTE_COLORS` in `paletteDefaults.ts`). Step-family rows are fixed (`primitive`, `skill`, `utility`, `control`).
