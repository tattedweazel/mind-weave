...
import { projectDeleteConfirmMessage } from '../../domain/workflowProjectMembership';

type WorkflowProjectDeleteControlProps = {
    projectName: string;
...
export function WorkflowProjectDeleteControl({
    projectName,
    workflowCount,
    disabled = false,
    onConfirmDelete,
}: WorkflowProjectDeleteControlProps) {
    ...
    const handleDelete = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            /* parent may surface errors; keep confirm open */
        } finally {
            setBusy(false);
        }
    };
    ...
    if (confirming) {
        return (
            ...
            >
                <span className="text-[10px] font-medium text-red-500 leading-tight truncate" title={projectDeleteConfirmMessage(projectName, workflowCount)}>
                    {projectDeleteConfirmMessage(projectName, workflowCount)}
                </span>
                <button
                    type="button"
                    disabled={busy}
