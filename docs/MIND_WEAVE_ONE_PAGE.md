...
**Projects** organize workflow definitions into **folders**. Each user has a seeded **Shared** folder; additional projects can be created. Use the **Enable in Sandbox** checkbox on each project row (opt-in; **Shared** is always available in Sandbox) to control which folders appear when picking creature brains or region triggers. Workflows can be **moved** between projects, **imported** as JSON into a project, or **exported** from the editor toolbar. List ordering can follow **last updated** or **name**, with optional **filtering** by name prefix.

**Board projects** (Sandbox left sidebar) organize saved boards the same way—independent folders with a seeded **Shared** row, sort/filter, move, delete project (cascade when non-empty), and delete individual boards from the sidebar or toolbar.

**Execution safety** applies per run: optional saved **`execution_limits`** and optional per-run overrides (TTL, maximum step count, maximum list size for **For Loop** inputs, nested **Workflow** depth) merge under operator-configured ceilings; **`GET /api/v1/workflow-execution-limits/`** exposes those caps to the SPA. Before **`POST …/run`** and **`POST …/runs`**, the API runs **preflight** static budgeting (For Loop counts from list primitives and caps): it **hard-rejects** only when a defensible estimate **clearly** exceeds **`max_node_executions`**, and returns **advisory warnings** (with UI confirm + **`acknowledge_preflight_warnings`**) when list sources or nested workflows make the estimate uncertain—so limits are not arbitrary surprises mid-run. **Try / Catch** groups a **`try`** path and **`catch`** path with structured **`ok`** / **`error`** dictionaries; streamed runs can surface **`handled_by_try_catch`** when a failure is routed to **`catch`**. **For Loop** supports **sequential**, **parallel** (legacy **`parallel_iterations`**), or **batched** iteration with optional **`summary`** aggregation.
...
The canvas is an interactive graph. **Nodes** expose **handles** for inputs and outputs. **Edges** connect handles. **Workflow palettes** persist per-handle hex overrides (`colors` JSON); labels and taxonomy come from **`shared/workflow_graph_step_kinds.json`**, and **`GET`/create/update palette** responses expose derived **`effective_colors`** and **`warnings`** when keys are stale. **Effective editor palette** precedence is defined on the server: **`GET /api/v1/palettes/resolve`** (optional **`workflow_id`**) applies **`workflow.palette_id`** when set and the palette is visible → **`User.settings.preferred_editor_palette_id`** → **Default** (slug **`default`** or equivalent). The SPA calls **`/resolve`** for authoritative colors and falls back to list + local defaults only when the request fails. There is **no** separate “activate palette” mutation route—preferred palette remains **`PUT /auth/me`**. Rendering uses **`effective_colors`** from the palette payload (explicit override → family fallback → shipped default **`any`**).
