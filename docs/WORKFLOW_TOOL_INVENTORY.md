...
|----------------|----------|-----------------|-----------------|
| `basic_conditional` | Branch on condition | `condition`, **`true`** / **`false`** (trigger + passthrough) | If/else style flows. **`true`** / **`false`** forward the resolved **condition** on the taken branch (separate edges for trigger vs data). |
| `is`, `gt`, `lt`, `gte`, `lte` | Compare | Value inputs + **`true`** / **`false`** branches | Comparisons. **`true`** / **`false`** forward **Input A** on the taken branch. |
| `is_empty` | True when value is `[]` or `{}` | `value` (list or dictionary) + **`true`** / **`false`** branches | Branch on empty collection; forward the wired **value** on the taken branch instead of rewiring upstream for branch-local data. |
| `between` | Range test | `low`, `value`, `high` + branches | Inclusive range. **`true`** / **`false`** forward **value** on the taken branch. |
...
**Suggested usage:** For branching controls, only **one** of **`true`** / **`false`** runs per evaluation. Each branch handle can schedule downstream steps (**`trigger`**) and optionally forward the control’s primary wired value on the taken branch (**data** edges to named inputs — use separate edges rather than rewiring upstream through the conditional). **For Loop End** must reference the correct **`for_loop_id`** and use **named** `target_handle` values matching your export keys. **Runtime dictionary keys come from each export edge’s `target_handle`**, not from `data.exports` alone; the workflow editor keeps wired export edges aligned when you rename export keys in the inspector. Hand-edited graphs with mismatched `target_handle` / `exports` fail validation.
